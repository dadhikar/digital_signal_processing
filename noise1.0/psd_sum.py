...
path = " "
v, PSD_ttl = np.loadtxt(path + os.sep + "PSD_SUM.txt", skiprows=1,
                        usecols=[0, 1], unpack=True)
alpha = np.loadtxt(path + os.sep + "alpha.txt", skiprows=1, usecols=1,
                   unpack=True)
...
ax1.text(0.4, 0.8, r'K-TCNQ_B4S2', transform=ax1.transAxes,
         bbox={'facecolor': 'skyblue', 'alpha': 0.8, 'pad': 10})

ax2 = ax1.twinx()
ax2.plot(v, alpha, '-o', color='g', label=r"$\alpha$")
ax2.set_ylabel(r'$\alpha$', fontsize=20)
ax2.legend(loc='lower center', bbox_to_anchor=(0.15, 0.7), shadow=False,
